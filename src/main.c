...
int main(void)
{
  ...
  const char *conninfo = getenv("MAILROOM_DATABASE_URL");
  if (!conninfo)
  {
    log_printf("MAILROOM_DATABASE_URL not set");
    return EXIT_FAILURE;
  }

  const char *hmac_secrethex = getenv("MAILROOM_SECRET_KEY");
  if (!hmac_secrethex)
  {
    log_printf("MAILROOM_SECRET_KEY not set");
    return EXIT_FAILURE;
  }

  if (!is_valid_hmac_secrethex(hmac_secrethex))
  {
    log_printf("MAILROOM_SECRET_KEY must be a 64-character hex string");
    return EXIT_FAILURE;
  }
  ...
  if (hmac_secretlen == 0)
  {
    log_printf("failed to decode MAILROOM_SECRET_KEY");
    return EXIT_FAILURE;
  }

  const char *channel_name = getenv("MAILROOM_CHANNEL_NAME");
  if (!channel_name)
  {
    channel_name = ENV_DB_CHANNEL_NAME;
  }

  const char *queue_name = getenv("MAILROOM_QUEUE_NAME");
  if (!queue_name)
  {
    queue_name = ENV_DB_QUEUE_NAME;
  }

  int healthcheck_ms = parse_env_int("MAILROOM_HEALTHCHECK_INTERVAL", ENV_DB_HEALTHCHECK_INTERVAL);
  int timeout_ms = parse_env_int("MAILROOM_BATCH_TIMEOUT", ENV_BATCH_TIMEOUT);

  if (healthcheck_ms < timeout_ms)
  {
    log_printf("MAILROOM_HEALTHCHECK_INTERVAL must be greater than MAILROOM_BATCH_TIMEOUT, or equal to it");
    return EXIT_FAILURE;
  }

  int batch_limit = parse_env_int("MAILROOM_BATCH_LIMIT", ENV_BATCH_LIMIT);
